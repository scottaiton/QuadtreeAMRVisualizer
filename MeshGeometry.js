class MeshGeometry {
  constructor(gl, mesh) {
    if (mesh.patches.length > 0) {
      ...
      this.generateLineIndexes(gl, mesh.patches);
    } else {
      this.empty = true;
    }
  }
  ...
  generateVertexIndexes(gl, patches) {
    ...
    this.vertexIndexArray = pos_array;
    this.vertexIndexBuffer.itemSize = 1;
    this.vertexIndexBuffer.numItems = pos_array.length;
  }
  generateLineIndexes(gl, patches) {
    this.lineIndexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.lineIndexBuffer);

    var pos_array = new Array((this.vertexIndexArray.length / 3) * 6);
    for (var i = 0; i < this.vertexIndexArray.length / 3; i++) {
      var a = this.vertexIndexArray[3 * i];
      var b = this.vertexIndexArray[3 * i + 1];
      var c = this.vertexIndexArray[3 * i + 2];
    ...
    this.lineIndexBuffer.itemSize = 1;
    this.lineIndexBuffer.numItems = pos_array.length;
    this.lineIndexBuffer.numPatchItems =
      this.vertexIndexBuffer.numPatchItems * 2;
  }
}
